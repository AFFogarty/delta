...
trait DeltaTableOperations extends AnalysisHelper { self: DeltaTable =>
  ...
  protected def makeUpdateTable(
    ...
    val updateColumns = setColumns.map { x =>
      UnresolvedAttribute.quotedString(x._1)
    }
    val updateExpressions = setColumns.map { x =>
      x._2.expr
    }
    val condition = onCondition.map { _.expr }
    UpdateTable(target.toDF.queryExecution.analyzed, updateColumns, updateExpressions, condition)
  }
