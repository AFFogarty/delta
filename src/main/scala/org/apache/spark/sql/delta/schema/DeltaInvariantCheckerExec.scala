 ...
 */
case class DeltaInvariantCheckerExec(child: SparkPlan, invariants: Seq[Invariant])
  ...
  private def buildExtractors(invariant: Invariant): Option[Expression] = {
    ...
    if (invariant.column.length == 1) {
      ...
      topLevelRefOpt.flatMap { topLevelRef =>
        ...
        try {
          val nested = invariant.column.tail.foldLeft(boundTopLevel) {
            case (e, fieldName) =>
              e.dataType match {
                case StructType(fields) =>
                  val ordinal =
                    fields.indexWhere(f => SchemaUtils.DELTA_COL_RESOLVER(f.name, fieldName))
                  if (ordinal == -1) {
                    throw new IndexOutOfBoundsException(
                      s"Not nullable column not found in struct: " +
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    val boundRefs = invariants.map { invariant =>
      CheckDeltaInvariant(
        buildExtractors(invariant).getOrElse(Literal(null, NullType)),
        invariant)
    }
